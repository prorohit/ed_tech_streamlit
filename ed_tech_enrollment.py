...
class Enrollment:
            ...
            if user_info["courses_enrolled"] is not None:
                ...
                for index, course in enumerate(courses_array):
                    if str(course["course_id"]) == str(course_id):
                        ...
                        return self.update_course_info_for_learner_user_st(user_id, courses_array)
    ...
    def update_course_info_for_learner_user_st(self, user_id, courses):
        record_found = False
            ...
            if user_info["courses_to_be_tought"] is not None:
                ...
                for index, course in enumerate(courses_array):
                    if str(course["course_id"]) == str(course_id):
                        course_found = True
                        courses_array.remove(course)
                        return self.update_course_info_for_instructor_st(user_id, courses_array)
    ...
    def update_course_info_for_instructor_st(self, user_id, courses):
        record_found = False
        for index, user in enumerate(self.instructor.users_database):
            if str(user["user_id"]) == str(user_id):
                ...
                self.instructor.users_database[index] = dict_user
                return "Record deleted successfully"
